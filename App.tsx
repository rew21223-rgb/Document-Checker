        ...
        if (isOnline && scriptUrl) {
            ...
            try {
                if (wasTypeChanged) {
                    ...
                    const newDocuments = getRequiredDocuments(updatedDetails.memberType).reduce((acc, doc) => ({ ...acc, [doc.name]: false }), {});
        ...
        if (isOnline && scriptUrl) {
             ...
             normalizedImports.forEach(imported => {
                 ...
                 if (existingIndex !== -1) {
                     ...
                     if (existing.memberType !== imported.memberType) {
                          const requiredDocs = getRequiredDocuments(imported.memberType);
                          finalDocuments = requiredDocs.reduce((acc, doc) => ({ ...acc, [doc.name]: false }), {});
                          finalHistory = []; 
                     }
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <Dashboard stats={
                        (() => {
                             ...
                             const initial = Object.values(MemberType).reduce((acc, t) => ({...acc, [t]: {total:0, complete:0, pending:0, incomplete:0}}), {} as any);
                            ...
                            <MemberTable
                                members={
                                    members.filter(m => {
                                        ...
                                        const isC = getCoreRequiredDocuments(m.memberType).every(d => m.documents[d.name]);
